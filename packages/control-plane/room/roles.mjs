...
const SHARED = [
  ...
  '  .spicyspec/runs/<spec>-r<run>-<startedMs>/   one directory per run: packet.md is the',
  '                                  assignment, stream.jsonl is what the worker actually did.',
  '                                  NEWEST means the largest trailing <startedMs>, NOT the last',
  '                                  name in a listing — a lexical sort puts 014-r1-<older> after',
  '                                  008-r1-<newer> and has already produced a confident wrong',
  '                                  answer about which spec was live. Sort by mtime or by that',
  '                                  number, and say which run you read.',
